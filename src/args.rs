...
pub struct Args {
    ...
    #[arg(short = 'e', long)]
    connect: Vec<String>,
    /// Endpoints to listen on. [default: tcp/localhost:7447]
    #[arg(short, long)]
    listen: Vec<String>,
...
impl From<&Args> for Config {
    fn from(args: &Args) -> Self {
        ...
        if !args.connect.is_empty() {
            ...
            // default to tcp/localhost:7447 if no config file and no --connect argument
            config
                .insert_json5(
                    "connect/endpoints",
                    &json!([DEFAULT_ZENOHD_LOCATOR]).to_string(),
                )
                .unwrap();
        }
